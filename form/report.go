...
type ReportBlockForm struct {
	...
	Number   string      `form:"number" binding:"required" json:"number"`     // 链编号
	Id       string      `form:"id" binding:"required" json:"id"`             // 节点id
	Height   int         `form:"height" binding:"required" json:"height"`     //当前区块高度
	...
	Interval int         `form:"interval" binding:"required" json:"interval"` //出块间隔
	Trans    int         `form:"trans" binding:"required" json:"trans"`       //交易数量
	Size     int         `form:"size" binding:"required" json:"size"`         //区块大小
...
type ReportFlowForm struct {
	...
	Id     string `form:"id" binding:"required" json:"id"`         // 节点id
	Time   string `form:"time" binding:"required" json:"time"`     //当前产生时间
	In     int    `form:"in" binding:"required" json:"in"`         //下行带宽
...
type ShardInfoForm struct {
	...
	SharedNum int `form:"sharedNum" binding:"required"`
